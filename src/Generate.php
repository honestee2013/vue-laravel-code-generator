...
class Generate extends Command
{
    ...
    public function createVueListTemplate($data){
      ...
      // Check if file already exists. If it does ask if we want to overwrite
      if ($client->exists($data['singular'].'.vue')) {
        if (!$this->confirm($data['singular'].'.vue already exists. Would you like to overwrite this component?')) {
          return false;
        }
      } 
      ...
      $client->put($data['singular'].'.vue', $vueTemplate );
    ...
    public function createRoutes($data){
      ...
      $routes = "\nRoute::apiResources([ '". $data['singular_lower'] 
      ."' => 'App\Http\Controllers\Honestee\VueCodeGen\\". $data['singular']."Controller' ]);\n";
    ...
    public function getFieldsData($singular, $plural){
      ...
      foreach ($data as $key) {
          ...
          if($type == 'enum'){
            $enumArray = $this->getEnumValues(strtolower($plural), $key->Field); // table, column
            $enumValues =  array_values($enumArray) ;
            $enumValues[0] = str_replace("('", "", $enumValues[0]) ; // clean enum begining and end
            $enumValues[ count( $enumValues) -1 ] = str_replace("')", "", $enumValues[ count( $enumValues) -1 ]) ;
            $fieldsArray[$i]['enumArray'] = $enumValues;
          }
    ...
    public function createFormData($plural){
      ...
      foreach ($data as $key) {
        ...
        $numericArray = ['tinyint','smallint','mediumint','int','bigint','decimal','float','double','bit'];
        
        $typeArr = explode("(", $key->Type, 2);
        $type = $typeArr[0];
        
        if (in_array($type,$numericArray)) {
          $vform.="\t\t\t\t\t\t<input type='number' ".$inputLength." class='form-control' placeholder='".ucfirst($key->Field)."' v-model='form.".$key->Field."'/>\n";
        }
        ...
        if (in_array($type,$stringArray)) {
          $vform.="\t\t\t\t\t\t<input type='text' ".$inputLength." class='form-control' placeholder='".ucfirst($key->Field)."' v-model='form.".$key->Field."'/>\n";
        }
        ...
        if (in_array($type,$textAreaArray)) {
          $vform.="\t\t\t\t\t\t<textarea ".$inputLength." class='form-control' style='min-height: 100px;' placeholder='".ucfirst($key->Field)."' v-model='form.".$key->Field."'/></textarea>\n";
        }
        
        $dateArray = ['date','timestamp','date','datetime','year'];
        
        if (in_array($type,$dateArray)) {
          $vform.="\t\t\t\t\t\t<input type='date' placeholder='".ucfirst($key->Field)."' v-model='form.".$key->Field."'/>\n";
        }
